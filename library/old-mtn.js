...
$(document).ready(function(){
	...
	//	$(this).find('#fake-width').width($(this).next().find('table').width( ) ) ;
		...
		$(".timetable-holder2.scroller0").scroll(function(){
    	  ...
    	  $("#fake-scroll.scroller0").scrollLeft($(this).scrollLeft());
  		});
  		$("#fake-scroll.scroller0").scroll(function(){
  		
   		 $(".timetable-holder2.scroller0").scrollLeft($(this).scrollLeft());
  		});
  		
  		$(".timetable-holder2.scroller1").scroll(function(){
			
    	  $("#fake-scroll.scroller1").scrollLeft($(this).scrollLeft());
  		});
  		$("#fake-scroll.scroller1").scroll(function(){
  		
   		 $(".timetable-holder2.scroller1").scrollLeft($(this).scrollLeft());
  		});
  		
  		
  		$(".timetable-holder2.scroller2").scroll(function(){
			
    	  $("#fake-scroll.scroller2").scrollLeft($(this).scrollLeft());
  		});
  		$("#fake-scroll.scroller2").scroll(function(){
  		
   		 $(".timetable-holder2.scroller2").scrollLeft($(this).scrollLeft());
  		});
	...
	$('table').each(function(i, tbody) {
		...
		var captionHeight = $(tbody).find('caption').height();
		 ...
		 $(tbody).find('tr').each(function(j, tr) {
			...
			if($(tr).find('th').width() > maxWidth) maxWidth = $(tr).find('th').width();
		 ...
		 $(tbody).find('tr').each(function(j, tr) {
		 	...
		 	if(!$(tr).hasClass('days-of-week-row')) {
		 		$(tr).find('th').width(maxWidth + 30);
		 	} else {
		 		$(tr).find('th:first').css('padding-left', maxWidth + 60);
		 		$(tr).find('td:first').css('height',$(tr).find('th:first').height() + 10 ).width(maxWidth + 30);
		 	}
